...
import javax.swing.*;
import com.smartrfid.tagtest.Panels.*;
...
public class gui {
    ...
    int[] ScanStartTime = new int[4];
    int[] CurrTime = new int[4];
    int ScanTime = 10;
    int maxLaps = 25;
    int lap;
    Date[] tagsDates = new Date[80];
    
    RFIDRenderer tableRenderer;
    
    Calendar current;
    javax.swing.Timer timer;
    
    private int SecondRowStartX = 50;
    private int ThirdRowStartX = 50;

    private int SecondRowStartY = 500;
    private int ThirdRowStartY = 500;
    ...
    public void changeIsReadingGray(boolean status){
        if (status){
    ...
    public void SetLapNum(int lapInput) {
		lapJLabel.setText(Integer.toString(lapInput));
		lap = lapInput;
	}
    
    public void resetFoundCells() {
		tableRenderer.resetData();
		scrollPane.repaint();
	}
    
    public void setCellFound(int row, int col) {
		tableRenderer.setCellState(row,col+4, true);
		current = Calendar.getInstance();
		current.setTime(new Date());
		current.add(Calendar.MILLISECOND, 500);
		tagsDates[row] = current.getTime();
		scrollPane.repaint();
	}
    
    public void startGUIWorker() {
        timer = new javax.swing.Timer(100, listener);
        timer.setInitialDelay(0);
        timer.start();
	}
    
    public void stopGUIWorker() {
		timer.stop();
	}
	...
	ActionListener listener = new ActionListener()
    {
        public void actionPerformed(ActionEvent event)
        {
        	...
        	for (int j = 0; j < tagsDates.length; j++) {
        		if (tagsDates[j] != null && currentDate.after(tagsDates[j])) {
        			tableRenderer.setCellState(j, lap+3, false);
        			scrollPane.repaint();
        		}
			}
    ...
    void CreateGui() {  
	    ...
	    tableRenderer = new RFIDRenderer();
